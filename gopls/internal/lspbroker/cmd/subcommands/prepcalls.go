...
// RunPrepCalls implements `gopls lspcli prep-calls [<symbol>] --in <file>[:line[:col]]`.
//
// It returns (exitCode, error):
//
//	0  success, non-empty result
//	1  success, empty result (no call hierarchy items found)
...
func RunPrepCalls(ctx context.Context, args []string, flags DefFlags, cacheDir, selfPath string, w io.Writer) (exitCode int, err error) {
	...
	raw, err := bc.PrepareCallHierarchy(ctx, params)
	if err != nil {
		code := classifyError(err)
		return code, err
	}

	// prep-calls always outputs JSON (no text formatter for Phase 2).
	hasContent, err := format.HoverJSON(w, raw) // reuse null-check + raw print logic
	if err != nil {
		return 4, err
	}
	if !hasContent {
