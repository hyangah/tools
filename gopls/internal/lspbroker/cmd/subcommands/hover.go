...
// RunHover implements `gopls lspcli hover [<symbol>] --in <file>[:line[:col]]`.
//
// It returns (exitCode, error):
//
//	0  success, non-empty result
//	1  success, empty result (no hover info found)
...
func RunHover(ctx context.Context, args []string, flags DefFlags, cacheDir, selfPath string, w io.Writer) (exitCode int, err error) {
	...
	var hasContent bool
	if flags.JSON {
		hasContent, err = format.HoverJSON(w, raw)
	} else {
		hasContent, err = format.Hover(w, raw)
	}
	if err != nil {
		return 4, err
	}
