...
// RunImpl implements `gopls lspcli impl [<symbol>] --in <file>[:line[:col]]`.
//
// It returns (exitCode, error):
//
//	0  success, non-empty result
//	1  success, empty result (no implementations found)
...
func RunImpl(ctx context.Context, args []string, flags DefFlags, cacheDir, selfPath string, w io.Writer) (exitCode int, err error) {
	...
	locs, err := bc.Implementation(ctx, params)
	if err != nil {
