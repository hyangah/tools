...
// newTestSession creates a GoSession for a test module at the given root.
func newTestSession(t *testing.T, ctx context.Context, c *cache.Cache, root string) *goplscli.GoSession {
	t.Helper()
...
func TestGoSessionDefinition(t *testing.T) {
...
func TestGoSessionReferences(t *testing.T) {
	ctx := context.Background()
	root := testFiles(t)
	c := cache.New(nil)
	gs := newTestSession(t, ctx, c, root)
...
func TestGoSessionHover(t *testing.T) {
	ctx := context.Background()
	root := testFiles(t)
	c := cache.New(nil)
	gs := newTestSession(t, ctx, c, root)
...
func TestGoSessionDocumentSymbols(t *testing.T) {
	ctx := context.Background()
	root := testFiles(t)
	c := cache.New(nil)
	gs := newTestSession(t, ctx, c, root)
...
func TestGoSessionDiagnoseFile(t *testing.T) {
	ctx := context.Background()
	root := testFiles(t)
	c := cache.New(nil)
	gs := newTestSession(t, ctx, c, root)
	...
	for _, d := range diags {
		t.Logf("diagnostic: %s: %s", d.URI, d.Message)
	}
}

func TestGoSessionEnsureSyncedFingerprint(t *testing.T) {
	ctx := context.Background()
	root := testFiles(t)
	c := cache.New(nil)
	gs := newTestSession(t, ctx, c, root)
...
// TestGoSessionConcurrent verifies that multiple goroutines can use
// the same GoSession concurrently without races.
func TestGoSessionConcurrent(t *testing.T) {
	ctx := context.Background()
	root := testFiles(t)
	c := cache.New(nil)
	gs := newTestSession(t, ctx, c, root)
