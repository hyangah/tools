...
// benchSession creates a fully warmed ServerSession: cache, session, view,
// and file synced. The session is cleaned up when the benchmark finishes.
func benchSession(b *testing.B) (*goplscli.ServerSession, string) {
	b.Helper()
	ctx := b.Context()
	root := benchFiles(b)
	...
	if err != nil {
		b.Fatalf("NewInProcessServer: %v", err)
	}
