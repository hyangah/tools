...
// resolvePosition resolves the target position from a CLI request. If
// req.Symbol is set, it resolves the symbol name to a position. Otherwise
// it converts the CLI line:column to a protocol range.
func resolvePosition(ctx context.Context, gs *ServerSession, req *Request, uri protocol.DocumentURI) (protocol.Range, error) {
	if req.Symbol != "" {
	...
	return protocol.Range{Start: pos, End: pos}, nil
}
...
func handleLocations(ctx context.Context, h *CLIHandler, req *Request, method string) *Response {
	...
	rng, err := resolvePosition(ctx, gs, req, uri)
	if err != nil {
		return &Response{Error: err.Error()}
	}
...
func handleDiagnostics(ctx context.Context, h *CLIHandler, req *Request) *Response {
	...
	for i, d := range diags {
		results[i] = convertProtocolDiag(d, req.File, mapper)
	}
...
func mapperForURI(ctx context.Context, gs *ServerSession, uri protocol.DocumentURI) (*protocol.Mapper, error) {
