...
// TestServeMissingFile verifies that the dispatch returns an error when
// File is empty for methods that require it (everything except wsymbols).
func TestServeMissingFile(t *testing.T) {
	...
	if !strings.Contains(resp.Error, "file is required") {
		t.Errorf("expected 'file is required' error, got: %s", resp.Error)
	}
}
...
func TestServeRename(t *testing.T) {
	...
	// Rename "Greeting" to "Salutation" at its declaration (line 6, col 6).
	resp, err = goplscli.SendRequest(ctx, addr, &goplscli.Request{
	...
	if resp.Error != "" {
		t.Fatalf("rename error: %s", resp.Error)
	}
	if len(resp.RenameEdits) == 0 {
		t.Fatal("rename returned no edits")
	}
	// Should have edits in at least one file.
	t.Logf("Rename returned %d file edits", len(resp.RenameEdits))
	for _, fe := range resp.RenameEdits {
		t.Logf("  %s: %d edits", fe.File, len(fe.Edits))
		for _, e := range fe.Edits {
			if e.NewText != "Salutation" {
				t.Errorf("expected newText 'Salutation', got %q", e.NewText)
			}
		}
	}
}

func TestServeRenameMissingNewName(t *testing.T) {
	root := testFiles(t)
	c := cache.New(nil)
	addr := startTestServer(t, c)
	ctx := t.Context()
	...
	resp, err := goplscli.SendRequest(ctx, addr, &goplscli.Request{
		...
		Line:   6,
		Column: 6,
		// NewName intentionally omitted.
	})
	if err != nil {
		t.Fatalf("request: %v", err)
	}
	if resp.Error == "" {
		t.Error("expected error for rename without newName")
	}
	if !strings.Contains(resp.Error, "newName is required") {
		t.Errorf("expected 'newName is required' error, got: %s", resp.Error)
	}
}
