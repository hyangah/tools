...
// TestSnapshotLeakDetection creates a ServerSession, runs several queries, then
// calls Close with a timeout. If Close doesn't return within the timeout,
// the test fails, indicating a snapshot leak from missing defer release() calls.
func TestSnapshotLeakDetection(t *testing.T) {
	ctx := t.Context()
	root := testFiles(t)
	c := cache.New(nil)

	// Create the session so we control shutdown (not via newTestSession).
