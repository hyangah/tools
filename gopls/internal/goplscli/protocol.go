...
// It uses SymbolKind's fmt.Formatter (defined in protocol/enums.go).
func symbolKindString(k protocol.SymbolKind) string {
	return fmt.Sprint(k)
}

// convertProtocolDiag converts a [protocol.Diagnostic] to a [DiagnosticResult]
// using the given mapper for position conversion.
func convertProtocolDiag(d protocol.Diagnostic, file string, mapper *protocol.Mapper) DiagnosticResult {
