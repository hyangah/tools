...
// socketPath computes a deterministic socket path for a given gopls binary
// and suffix, using the same hash derivation as lsprpc's autoNetworkAddressPosix.
func socketPath(goplsPath, suffix string) string {
	...
	return filepath.Join(runtimeDir, fmt.Sprintf("%s-%s-%s.%s", basename, shortHash, suffix, user))
}

func cliSocketPath(goplsPath string) string { return socketPath(goplsPath, "cli") }
func lspSocketPath(goplsPath string) string { return socketPath(goplsPath, "daemon") }
...
func AutoConnect(ctx context.Context) (string, error) {
	...
	// Daemon is not running. Remove stale socket files if present.
	// Ignore ErrNotExist — the file may not exist.
	if err := os.Remove(cliAddr); err != nil && !os.IsNotExist(err) {
		return "", fmt.Errorf("removing stale CLI socket: %w", err)
	}
	// The spawned daemon needs to bind the LSP address; if a previous
	// daemon crashed, the socket file is left behind and net.Listen will fail.
	lspAddr := lspSocketPath(goplsPath)
	if err := os.Remove(lspAddr); err != nil && !os.IsNotExist(err) {
		return "", fmt.Errorf("removing stale LSP socket: %w", err)
	}
